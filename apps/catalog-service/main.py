...
app = FastAPI(
    ...
    description="Catalog service with RapidAPI Amazon and Kroger integration",
    version="1.0.0",
)
...
kroger_client = (
    KrogerAPIClient(client_id=kroger_client_id, client_secret=kroger_client_secret)
...
async def search_products(request: SearchRequest):
    """
    Search products using RapidAPI Amazon API and Kroger (if configured).
    """
    try:
        ...
        platforms_to_search = (
            request.filters.platforms if request.filters and request.filters.platforms else ["amazon"]
        )
...
# Compatibility endpoint to accept orchestrator-style payload
class OrchestratorRequirementSpec(BaseModel):
    product_type: str
...
class OrchestratorSearchRequest(BaseModel):
    requirement_spec: OrchestratorRequirementSpec
    limit: Optional[int] = 20
    marketplaces: Optional[List[str]] = None


@app.post("/api/v1/search/orchestrator", response_model=SearchResponse)
async def search_products_orchestrator(payload: OrchestratorSearchRequest):
    """
    Compatibility layer for orchestrator payload:
    {
        "requirement_spec": {...},
        "limit": 20,
    ...
    """
    rs = payload.requirement_spec

    # Build query string: product_type + key attributes/brands
    query_parts = [rs.product_type]
    if rs.brand_preferences:
        query_parts.extend(rs.brand_preferences)
    # Include attribute values (e.g., colors, sizes) to improve relevance
    if rs.attributes:
        for val in rs.attributes.values():
            if isinstance(val, (list, tuple)):
                query_parts.extend([str(v) for v in val])
            elif isinstance(val, dict):
                query_parts.extend([str(v) for v in val.values()])
    ...
    price_max = rs.price.get("max") if rs.price else None
    brands = rs.brand_preferences or None
    platforms = payload.marketplaces or rs.marketplaces or ["amazon"]
    ...
    search_request = SearchRequest(
        ...
        pagination={"page": 1, "size": payload.limit or 20},
        sort_by=None,
    )
...
async def get_product_details(product_id: str):
    ...
    try:
        ...
        product = Product(
            ...
            brand=details.get("product_details", {}).get("Brand")
            if isinstance(details.get("product_details"), dict)
            else None,
            price=price,
